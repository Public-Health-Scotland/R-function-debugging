...
# What happens in a function stays in a function, unless you say otherwise
create_agegroups(dataset = dummy_data)

# Assign to an object to reuse
dummy_data_grouped <- create_agegroups(dataset = dummy_data)
# Most functions can be "piped" for easier reading
...
create_quintile_data <- function(quint_number) {
  ...
  dummy_data2 <- dummy_data2 %>% 
    filter(quintile == quint_number) #filtering only quintile of interest
